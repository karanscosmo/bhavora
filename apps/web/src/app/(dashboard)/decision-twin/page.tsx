...
export default function DecisionTwinPage() {
  ...
  useEffect(() => {
    ...
    import('mapbox-gl').then((mapboxglModule) => {
      ...
      map = new mapboxgl.Map({
        ...
        style: 'mapbox://styles/mapbox/dark-v11', // Dark style for data visualization
      ...
      map.on('load', () => {
        ...
        // Add dynamic heatmap source
        map.addSource('impact-heat', {
          type: 'geojson',
          data: { type: 'FeatureCollection', features: [] }
        });

        map.addLayer({
          id: 'layer-impact-heat',
          type: 'heatmap',
          source: 'impact-heat',
          paint: {
  ...
  // Update heatmap data when sliders change
  useEffect(() => {
    if (!mapRef.current || !mapRef.current.isStyleLoaded()) return;
    
    // Create synthetic data points based on sliders
    const features: GeoJSON.Feature[] = [];
    
    // Population growth impact (spreads across city)
    if (popGrowth > 0) {
      for (let i = 0; i < popGrowth; i++) {
        features.push({
          type: 'Feature',
          geometry: { type: 'Point', coordinates: [77.5946 + (Math.random() - 0.5) * 0.4, 12.9716 + (Math.random() - 0.5) * 0.4] },
          properties: { intensity: 5 + (popGrowth / 10) }
        });
      }
    }
    
    // Industrial expansion (concentrated in specific zones)
    if (indExpansion > 0) {
       for (let i = 0; i < indExpansion * 2; i++) {
        features.push({
          type: 'Feature',
          geometry: { type: 'Point', coordinates: [77.65 + (Math.random() - 0.5) * 0.1, 12.85 + (Math.random() - 0.5) * 0.1] }, // ECity area
          properties: { intensity: 8 }
        });
        features.push({
          type: 'Feature',
          geometry: { type: 'Point', coordinates: [77.50 + (Math.random() - 0.5) * 0.1, 13.03 + (Math.random() - 0.5) * 0.1] }, // Peenya area
          properties: { intensity: 8 }
        });
      }
    }

    const source = mapRef.current.getSource('impact-heat') as GeoJSONSource;
    if (source) {
      source.setData({
  ...
  return (
    <div className="absolute inset-0 flex overflow-hidden select-none animate-fade-in">
      {/* Map Section (Left) */}
      <section className="flex-1 relative bg-gray-900 overflow-hidden">
        <div ref={mapContainerRef} className="absolute inset-0 z-0 w-full h-full" />
        
        {/* Animated Overlay (optional subtle effect over map) */}
