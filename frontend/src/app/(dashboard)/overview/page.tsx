...
export default function OverviewPage() {
  ...
  // Compute values
  const displayPopulation = (13.6 * (1 + popGrowth / 100)).toFixed(1);
  const displayEnergy = (4.2 * (1 + metrics.energyDemand / 100)).toFixed(2);
  const displayWater = Math.max(1, 18 - metrics.waterDemand).toFixed(1);
  const displayCarbon = (2.4 * (1 + metrics.carbonEmissions / 100)).toFixed(2);
  const displayJobs = (114.2 + metrics.jobsCreated).toFixed(1);
  const displayGdp = (8.4 + (indExpansion * 0.1) + (popGrowth * 0.05)).toFixed(1);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        ...
        <div className="flex gap-3">
          ...
          <button 
            ...
            className="bg-primary text-on-primary px-6 py-2 rounded-lg font-bold text-sm shadow-md flex items-center gap-2 hover:opacity-90 transition-all cursor-pointer"
          >
